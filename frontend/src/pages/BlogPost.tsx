  ...
  return (
    <div className="max-w-4xl mx-auto p-6 text-gray-800">
      ...
      <p className="text-sm text-gray-500 mb-6">
        {new Date(post.created_at).toLocaleDateString()} | Tags: {post.tags} {post.author && `| By ${post.author}`}
      </p>
      ...
      <div className="flex justify-between border-t pt-6 mt-6 text-sm">
        {prevPost ? (
          <Link to={`/blog/${prevPost.slug}`} className="text-blue-600 hover:underline">
            ← {prevPost.title}
          </Link>
        ) : <span />}

        {nextPost ? (
          <Link to={`/blog/${nextPost.slug}`} className="text-blue-600 hover:underline ml-auto">
            {nextPost.title} →
          </Link>
        ) : <span />}
      </div>
